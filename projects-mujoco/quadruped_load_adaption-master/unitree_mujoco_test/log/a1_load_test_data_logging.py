...
with mujoco.viewer.launch_passive(model, data) as viewer:
    start = time.time()
    while viewer.is_running() and steps < 800:
        
        steps += 1
        time_step.append(steps * dt)
        ...
        log_base_lin_x.append(body_lin_vel[0])
        log_base_lin_y.append(body_lin_vel[1])
        log_base_lin_z.append(body_lin_vel[2])

        body_ang_vel = quaternion_rotate(
            inverse_quaternion(np.array(data.qpos[3:7])), np.array(data.qvel[3:6])
        )  # 机器人的角速度（在机身坐标系）
        log_base_ang_z.append(body_ang_vel[2])
...
# plot command[0] and base_lin_x with time_step as x-axis
fig, axs = plt.subplots(2, 2)
a = axs[0,0]
command_x_list = [command[0]] * len(time_step)
command_y_list = [command[1]] * len(time_step)
command_w_list = [command[2]] * len(time_step)
a.plot(time_step, log_base_lin_x, label="base_lin_x")
a.plot(time_step, command_x_list, label="command_x")
a.set(xlabel='time [s]', ylabel='base linear velocity (x)', title='Base Linear Velocity (x)')
a.legend()

b = axs[0,1]
b.plot(time_step, log_base_lin_y, label="base_lin_y")
b.plot(time_step, command_y_list, label="command_y")
b.set(xlabel='time [s]', ylabel='base linear velocity (y)', title='Base Linear Velocity (y)')
b.legend()

c = axs[1,0]
c.plot(time_step, log_base_ang_z, label="base_ang_z")
c.plot(time_step, command_w_list, label="command_w")
c.set(xlabel='time [s]', ylabel='base angular velocity (z)', title='Base Angular Velocity (z)')
c.legend()

d = axs[1,1]
d.plot(time_step, log_base_lin_z, label="base_lin_z")
d.set(xlabel='time [s]', ylabel='base linear velocity (z)', title='Base Linear Velocity (z)')
d.legend()
